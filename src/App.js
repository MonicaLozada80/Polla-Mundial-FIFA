...
// Bloqueo solo manual desde el Admin (sin bloqueo automático por fecha)
...
export default function App() {
  ...
  const [editPredictions, setEditPredictions] = useState({}); // {matchId: {h,a}}
  const [predStatus, setPredStatus] = useState({}); // {matchId: "idle"|"saving"|"saved"|"error"}
  ...
  // Edición LOCAL del pronóstico (no guarda todavía, solo actualiza el campo en pantalla)
  function handlePredChange(matchId, side, val) {
    ...
    // Si el usuario vuelve a editar después de guardar, quitamos el check de "guardado"
    setPredStatus(prev => ({...prev, [matchId]: "idle"}));
  }

  // Guarda el pronóstico de UN partido al hacer clic en el botón "Guardar"
    ...
    <div style={{minHeight:"100vh",background:BG,color:"white",fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{padding:14,maxWidth:700,margin:"0 auto"}}>
        ...
        {view==="predicciones"&&filteredMatches.map(m=>{
          ...
          return (
            <div key={m.id} style={{background:CARD,borderRadius:16,padding:"14px 16px",marginBottom:10,border:`1px solid ${locked?"rgba(255,255,255,0.08)":hasUnsavedChanges?"rgba(192,132,252,0.6)":GC[m.g]+"40"}`,opacity:locked&&!pred.h?0.6:1}}>
                ...
                <div style={{display:"flex",justifyContent:"center",marginTop:12}}>
                  <button onClick={()=>savePredictionClick(m.id)} disabled={status==="saving"}
                    style={{
                      ...
                      background: status==="saved" ? "#22c55e" : status==="error" ? "#ef4444" : status==="saving" ? "rgba(124,58,237,0.4)" : (hasUnsavedChanges?"linear-gradient(90deg,#7c3aed,#a855f7)":"rgba(255,255,255,0.08)"),
                      color: status==="idle"&&!hasUnsavedChanges ? "#c4b5fd" : "white"
                    }}>
        ...
        {view==="admin"&&user===ADMIN&&(
          <div>
            ...
            <div style={{background:CARD,borderRadius:16,padding:18,marginBottom:16,border:"1px solid rgba(192,132,252,0.4)"}}>
              ...
              <p style={{color:"#c4b5fd",fontSize:12,margin:"0 0 14px"}}>Agrega los partidos de Dieciseisavos, Octavos, Cuartos, Semifinal y Final cuando se conozcan los clasificados.</p>
